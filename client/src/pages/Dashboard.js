  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#0f172a] transition-all duration-500 overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 relative z-10">
        {/* Header */}
        <motion.div initial="hidden" animate="visible" variants={cardAnimation} className="mb-12">
          <h1 className="text-4xl font-extrabold text-[#1e293b] dark:text-white">
            Welcome back, <span className="text-[#e67e22]">{user?.name || "Explorer"}</span> 👋
          </h1>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            >
              <h2 className="text-xl font-bold text-[#1e293b] dark:text-white mb-6 flex items-center gap-2">
                <span className="w-2 h-6 bg-[#e67e22] rounded-full"></span> Quick Actions
              </h2>
              <div className="grid md:grid-cols-2 gap-6">
                <Link to="/quiz">
                  ...
                  >
                    ...
                    <h3 className="font-bold text-lg text-[#1e293b] dark:text-white">Career Quiz</h3>
                    <p className="text-gray-500 dark:text-gray-400 text-sm">Update your personality profile</p>
                  </motion.div>
                </Link>
                ...
                <Link to="/colleges">
                  ...
                  >
                    ...
                    <h3 className="font-bold text-lg text-[#1e293b] dark:text-white">Find Colleges</h3>
                    <p className="text-gray-500 dark:text-gray-400 text-sm">Browse govt institutions</p>
                  </motion.div>
                </Link>
              </div>
            </motion.div>
            ...
            {recommendations.length > 0 && (
              ...
              >
                <h2 className="text-xl font-bold text-[#1e293b] dark:text-white mb-6 flex items-center gap-2">
                  <span className="w-2 h-6 bg-[#3498db] rounded-full"></span> Suggested Paths
                </h2>
                  ...
                  {recommendations.map((stream, index) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <h3 className="font-bold capitalize text-lg">{stream} Stream</h3>
                        <p className="text-xs text-blue-100 font-medium uppercase tracking-wider">Top Match for you</p>
                      </div>
                      <Link to={`/courses?stream=${stream}`}
            ...
            {recentQuizzes.length > 0 && (
              ...
              >
                <h2 className="text-xl font-bold text-[#1e293b] dark:text-white mb-6">History</h2>
                <div className="space-y-4">
                  {recentQuizzes.map((quiz, index) => (
                    <div key={index} className="p-5 rounded-2xl bg-gray-50 dark:bg-[#0f172a] border border-gray-100 dark:border-gray-700 flex flex-col md:flex-row justify-between gap-4">
                      <div>
                        <h3 className="font-bold text-[#1e293b] dark:text-white">{quiz.quizType}</h3>
                        <p className="text-xs font-bold text-[#e67e22] mt-1">{new Date(quiz.completedAt).toLocaleDateString('en-GB')}</p>
                      </div>
                      <div className="md:text-right">
                        <p className="text-xs text-gray-500 mb-1">Top Matches</p>
                        <div className="flex flex-wrap md:justify-end gap-2">
                          {quiz.recommendations.map((r, i) => (
                            <span key={i} className="px-3 py-1 bg-[#3498db]/10 text-[#3498db] text-xs font-bold rounded-lg border border-[#3498db]/20">
                              {r}
                            </span>
                          ))}
                        </div>
          ...
          <div className="space-y-8">
            {/* Profile Status */}
            <motion.div custom={4} initial="hidden" animate="visible" variants={cardAnimation}
              className="bg-white dark:bg-[#1e293b] rounded-[2.5rem] p-8 shadow-xl border-t-4 border-[#e67e22]"
            >
              <h3 className="font-bold text-[#1e293b] dark:text-white mb-6 text-center">Profile Strength</h3>
              <div className="space-y-5">
                {[
                  { label: "Basic Info", status: "Done", color: "text-green-500" },
                  { label: "Academics", status: "Update", color: "text-[#3498db]", link: "/profile" },
                  { label: "Assessment", status: "Quiz", color: "text-[#e67e22]", link: "/quiz" },
                ].map((item, i) => (
                  <div key={i} className="flex justify-between items-center bg-gray-50 dark:bg-[#0f172a] p-3 rounded-xl">
                    <span className="text-sm font-semibold text-gray-600 dark:text-gray-400">{item.label}</span>
                    {item.link ? (
                      <Link to={item.link} className={`text-xs font-black uppercase ${item.color} hover:underline`}>
                        {item.status} →
                      </Link>
                    ) : (
                      <span className="text-xs font-black uppercase text-green-500">✓ Completed</span>
                    )}
                  </div>
                ))}
              </div>
            ...
            {/* Saved Items */}
            <motion.div custom={5} initial="hidden" animate="visible" variants={cardAnimation}
            ...
            >
              ...
              <h3 className="font-bold mb-6 relative z-10 text-xl">Saved Library</h3>
              <div className="grid grid-cols-2 gap-4 relative z-10">
                <div className="bg-white/10 backdrop-blur-md p-4 rounded-2xl text-center">
                  <p className="text-3xl font-black">{savedColleges.length}</p>
                  <p className="text-[10px] uppercase font-bold tracking-widest text-blue-200">Colleges</p>
                </div>
                <div className="bg-white/10 backdrop-blur-md p-4 rounded-2xl text-center">
                  <p className="text-3xl font-black">0</p>
                  <p className="text-[10px] uppercase font-bold tracking-widest text-blue-200">Courses</p>
                </div>
              </div>
            </motion.div>

            {/* Deadlines */}
            <motion.div custom={6} initial="hidden" animate="visible" variants={cardAnimation}
              className="bg-[#e67e22] rounded-[2.5rem] p-8 text-white shadow-xl"
            >
              <h3 className="font-bold mb-2 flex items-center gap-2">
                <span className="animate-pulse">🔔</span> Notifications
              </h3>
              <p className="text-sm font-medium text-orange-50">No upcoming deadlines. We'll alert you for new admission dates.</p>
            </motion.div>
          </div>
        </div>
      </div>
