...
profileSchema.virtual("completionDetails").get(function () {
  ...
  if (
    this.careerPreferences.interests &&
    this.careerPreferences.interests.length > 0
  )
    completed++;
  ...
  if (this.academicInfo.subjects && this.academicInfo.subjects.length > 0)
    completed++;
...
profileSchema.index({
  "academicInfo.stream": 1,
  "academicInfo.currentClass": 1,
});
